...
export default function CommunityPostDetailClient({ postId }: CommunityPostDetailClientProps) {
  ...
  async function submitComment(event: FormEvent<HTMLFormElement>) {
    ...
    setMessage("");
    try {
      const response = await cfetch(`/api/community/posts/${encodeURIComponent(postId)}/comments`, {
        method: "POST",
        credentials: "include",
        headers: { "Content-Type": "application/json" },
  ...
  async function submitCommentEdit(commentId: string) {
    ...
    setMessage("");
    try {
      const response = await cfetch(`/api/admin/community-comments/${encodeURIComponent(commentId)}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        credentials: "include",
        body: JSON.stringify({ content }),
  ...
  async function doDeleteComment() {
    ...
    setMessage("");
    try {
      const response = await cfetch(`/api/admin/community-comments/${encodeURIComponent(deleteCommentId)}`, {
        method: "DELETE",
        credentials: "include",
      });
      const data = await response.json().catch(() => ({}));
