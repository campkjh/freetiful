...
export default function CommunityComposeModal({
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await cfetch("/api/category-groups");
      ...
      try {
        const me = await cfetch("/api/auth/me", { credentials: "include" });
    ...
    (async () => {
      try {
        const r = await cfetch("/api/gifs?probe=1");
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        const res = await cfetch(`/api/gifs?q=${encodeURIComponent(q)}`);
        const data = await res.json();
        if (!alive) return;
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      cfetch(url, { credentials: "include" })
        .then((r) => r.json())
        .then((d) => {
          if (!alive) return;
  ...
  useEffect(() => {
    ...
    let alive = true;
    cfetch("/api/ox-questions/pick?level=categories")
      .then((r) => r.json())
      .then((d) => {
  ...
  function toggleQuiz() {
    ...
    setQuizOn(true);
    pickFree();
    cfetch("/api/community/quiz-quota", { credentials: "include" })
