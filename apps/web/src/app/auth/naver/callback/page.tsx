...
import { consumeAuthReturnTo, consumeNaverRedirectUri, getOAuthRedirectUri } from '@/lib/auth/oauth';
...
function NaverCallbackInner() {
  ...
  useEffect(() => {
    ...
    if (code && state) {
      called.current = true;
      naverLogin(code, state, consumeNaverRedirectUri() || getOAuthRedirectUri('naver'));
    } else {
      called.current = true;
