...
export default function AnalyticsPage() {
  ...
  useEffect(() => {
    if (!authUser) return;
    apiClient.get('/api/v1/pro/analytics')
      .then((res) => {
        const d = res.data;
        if (d?.weeklyViews != null) {
          setStats({ weeklyViews: d.weeklyViews, weeklyInquiries: d.weeklyInquiries ?? 0, conversionRate: d.conversionRate ?? '0%' });
        }
        if (Array.isArray(d?.daily)) {
          setWeeklyData(d.daily.map((item: any) => ({ day: item.day, views: item.views })));
        }
      })
    ...
    <div className="bg-gray-50 min-h-screen">
      ...
      <div className="px-4 mt-4 grid grid-cols-2 gap-3">
        <div className="card p-4">
          ...
          <p className="text-2xl font-black text-gray-900 mt-1">{stats?.weeklyViews ?? 388}</p>
        ...
        <div className="card p-4">
          ...
          <p className="text-2xl font-black text-gray-900 mt-1">{stats?.weeklyInquiries ?? 12}</p>
          <p className="text-[10px] text-green-500 flex items-center gap-0.5">
        ...
        <div className="card p-4">
          ...
          <p className="text-2xl font-black text-gray-900 mt-1">{stats?.conversionRate ?? '3.1%'}</p>
            ...
            {weeklyData.map(({ day, views }) => (
              <div key={day} className="flex-1 flex flex-col items-center gap-1">
                ...
                <div
                  ...
                  style={{ height: `${(views / displayMaxViews) * 100}%`, minHeight: '4px' }}
                />
