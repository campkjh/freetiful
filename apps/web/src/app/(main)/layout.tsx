...
export default function MainLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-surface-50">
      ...
      {/* Login Modal */}
      {showLoginModal && (
        <div className="fixed inset-0 z-[100] flex items-end justify-center bg-black/40" onClick={() => router.push('/main')}>
          <div
          ...
          >
            ...
            <div className="space-y-2.5">
              <button
                onClick={() => { localStorage.setItem('freetiful-logged-in', 'true'); localStorage.setItem('freetiful-user', JSON.stringify({ name: '', provider: 'kakao', createdAt: Date.now() })); localStorage.setItem('userRole', 'general'); window.location.href = '/onboarding'; }}
                className="w-full flex items-center justify-center gap-3 bg-[#FEE500] text-[#191919] font-semibold py-3.5 rounded-xl active:scale-[0.98] transition-transform"
              >
              ...
              <button
                onClick={() => { localStorage.setItem('freetiful-logged-in', 'true'); localStorage.setItem('freetiful-user', JSON.stringify({ name: '', provider: 'naver', createdAt: Date.now() })); localStorage.setItem('userRole', 'general'); window.location.href = '/onboarding'; }}
                className="w-full flex items-center justify-center gap-3 bg-[#03C75A] text-white font-semibold py-3.5 rounded-xl active:scale-[0.98] transition-transform"
              >
              ...
              <button
                onClick={() => { localStorage.setItem('freetiful-logged-in', 'true'); localStorage.setItem('freetiful-user', JSON.stringify({ name: '', provider: 'google', createdAt: Date.now() })); localStorage.setItem('userRole', 'general'); window.location.href = '/onboarding'; }}
                className="w-full flex items-center justify-center gap-3 bg-white text-gray-700 border border-gray-200 font-semibold py-3.5 rounded-xl active:scale-[0.98] transition-transform"
              >
              ...
              <button
                onClick={() => { localStorage.setItem('freetiful-logged-in', 'true'); localStorage.setItem('freetiful-user', JSON.stringify({ name: '', provider: 'apple', createdAt: Date.now() })); localStorage.setItem('userRole', 'general'); window.location.href = '/onboarding'; }}
                className="w-full flex items-center justify-center gap-3 bg-black text-white font-semibold py-3.5 rounded-xl active:scale-[0.98] transition-transform"
              >
            ...
            <button onClick={() => router.push('/main')} className="w-full mt-4 text-[14px] text-gray-400 font-medium py-2 text-center">
              나중에 하기
            </button>
          </div>
